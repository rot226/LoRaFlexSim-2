# Expériences de mobilité du scénario B

Ce dossier rassemble les scripts utilisés pour reproduire les expériences axées sur la mobilité du scénario B de la campagne MNE3SD. La suite est organisée autour d'un petit ensemble de scénarios de mobilité canoniques et des utilitaires de post-traitement nécessaires pour transformer les sorties brutes du simulateur en figures prêtes à l’export.
...
- `--output` : fichier CSV de destination dans `results/mne3sd/scenario_b/`.
...
Les modules de tracé sont de fines surcouches qui agrègent les CSV produits par les scénarios et génèrent les figures du scénario B. L'interface partagée accepte :
...
- `--figures-dir` : répertoire de destination des figures exportées. Valeur par défaut : `figures/mne3sd/scenario_b/`.
...
- **Répartition du PDR par passerelle** (`figures/mne3sd/scenario_b/mobility_gateway/pdr_distribution_by_gateway/`) : graphique en barres empilées montrant la part de trafic collectée par chaque passerelle selon le modèle de mobilité et le nombre total de passerelles. Une répartition homogène signale une couverture bien équilibrée tandis qu'un segment dominant met en évidence un goulot d'étranglement.
- **Délai moyen downlink vs passerelles** (`figures/mne3sd/scenario_b/mobility_gateway/downlink_delay_vs_gateways/`) : courbes avec barres d'erreur reliant le nombre de passerelles au délai moyen des accusés de réception. La pente renseigne sur l'intérêt d'ajouter des passerelles supplémentaires pour réduire la latence downlink.
- **Comparaison RandomWaypoint/Smooth** (`figures/mne3sd/scenario_b/mobility_gateway/model_comparison/`) : nuage de points positionnant chaque configuration selon son PDR agrégé et son délai downlink moyen. Les annotations `n GW` rappellent le nombre de passerelles associé, ce qui aide à visualiser le compromis portée/latence entre les profils de mobilité.
...
python -m scripts.mne3sd.scenario_b.scenarios.run_mobility_speed_sweep \
    --profile fast --workers 4 --results results/mne3sd/scenario_b/mobility_speed_fast.csv
```

## Structure du répertoire

```
scripts/mne3sd/scenario_b/
├── README.md                # Ce guide
...
### Artefacts CSV
Toutes les données brutes et métriques agrégées doivent résider dans `results/mne3sd/scenario_b/`. Utilisez des noms explicites comme `urban_canyon_runs.csv` ou `rural_highway_summary.csv`. Lorsqu'un scénario produit plusieurs fichiers (par exemple métriques par passerelle et traces par nœud), créez un sous-dossier : `results/mne3sd/scenario_b/industrial_campus/gateway_load.csv`.
...
- `latency_s`, `delivery_ratio` : indicateurs clés utilisés dans les figures du scénario B.

### Artefacts graphiques
Exportez les figures dans `figures/mne3sd/scenario_b/`, en gardant des noms alignés sur la nomenclature d’export (ex. `figure_3_latency_vs_speed.pdf`). Placez les graphiques temporaires de débogage dans un sous-dossier dédié tel que `figures/mne3sd/scenario_b/debug/` pour ne pas les mélanger aux figures exportées.
...
```
python -m scripts.mne3sd.scenario_b.scenarios.<scenario_module> \
    --runs 20 \
    --duration 7200 \
    --seed 123 \
    --output results/mne3sd/scenario_b/<scenario_name>.csv
...
```
python -m scripts.mne3sd.scenario_b.plots.<figure_module> \
...
1. Exécutez chaque script de scénario requis jusqu'à remplir `results/mne3sd/scenario_b/` avec les CSV attendus.
...
3. Lancez les modules de tracé pour générer les figures exportées dans `figures/mne3sd/scenario_b/`.
4. Relisez les figures exportées localement avant de les intégrer au support d’export.

### Lanceur de batch

Vous pouvez exécuter toute la chaîne du scénario B en une seule commande grâce à `scripts/mne3sd/run_all_scenario_exports.py` :

```
python -m scripts.mne3sd.run_all_scenario_exports --target scenario_b
```
