...
run_cmd() {
  local subcommand=$1
  local raw_args=$2
  echo "[loraflexsim] ${subcommand} ${raw_args}"
  ...
  eval "\"${PYTHON_BIN}\" -m loraflexsim ${subcommand} ${raw_args}"
}
...
echo "Pipeline loraflexsim terminé (point d'entrée officiel recommandé)."
